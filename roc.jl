...
function roc(prob_values, labels; prob_thresh = collect(0:0.05:1))
    ...
    # auc[!, :fpr] = convert(Array{Union{Missing, Float64}}, auc[:, :fpr])
    df.sort!(auc, (order(:fpr, rev=false), order(:tpr, rev=false)))
    auc[!, :fpr_prev] = vcat(NaN, auc[1:(df.nrow(auc)-1), :fpr])
    auc[!, :tpr_prev] = vcat(NaN, auc[1:(df.nrow(auc)-1), :tpr])
    ...
    auc = sum(auc[isnan.(auc[:, :area]) .== false, :area])
